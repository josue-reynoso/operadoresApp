{{-- @if(count( json_decode($results->dataSet), true) > 0) --}}
    ...
    <script type="text/javascript">
                        ...
                        columnDefs: [
                            {
                                ...
                                render: function (data, type, full, meta) {
                                    map1.set(full[1], 1);
                                    $('#cantidadP').text('Cantidad: '+map1.size);
                                    //console.log(full)
                                    return (
                                        '<div class="d-inline-flex dropright">' +
                                            ...
                                            '<div class="dropdown-menu dropdown-menu-right justify-content-center navbar-shadow">' +
                                                '<a href="' + aUrl + '/edit/' + full[1] + '" class="dropdown-item">' +
                                                    {{--feather.icons['edit'].toSvg({ class: 'font-small-4 mr-50' }) + ( typeof MB === 'undefined' ? 'Editar' : 'Ver') +--}}
                                                    feather.icons['edit'].toSvg({ class: 'font-small-4 mr-50' }) + ( full[7] == 'MB Descartado' ? 'Ver' : (full[7] == 'MB Transito' ? 'Ver' : (full[7] == 'MB Deposito' ? 'Ver' : 'Editar'))) +
                                                '</a>' + (full[2]=='Producto'?
                                                ( typeof MB === 'undefined' ? '<a href="' + aUrl + (full[4]=='Defectuoso'?'/restore/':'/delete/') + full[1] + '" class="dropdown-item">' +
                                                    feather.icons['trash-2'].toSvg({ class: 'font-small-4 mr-50' }) + (full[4]=='Defectuoso'?'Restaurar':'Eliminar') +
                                                '</a>' : '') : '') +
                                            '</div>' +
                                        '</div>'
                                    );
        ...
        //createDataTable();//comentado evita que obtenga datos al cargar
        $(function () {
            'use strict';
